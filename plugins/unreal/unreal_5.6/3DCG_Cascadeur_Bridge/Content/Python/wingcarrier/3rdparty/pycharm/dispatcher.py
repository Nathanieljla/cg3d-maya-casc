"""dispatcher.py - Wing Carrier dispatcher for PyCharm

This script is the PyCharm equivalent of the Antigravity/VS Code dispatcher.
It is invoked by a PyCharm External Tool (Settings → Tools → External Tools)
which passes the active file path as a command-line argument via the built-in
macro ``$FilePath$``, since PyCharm has no Python API for reading the active
editor state from an external script.

Usage:
    python dispatcher.py <file_path>

Where <file_path> is the absolute path to the file currently open in the
PyCharm editor, provided via the ``$FilePath$`` External Tool macro.
"""
...
# ---------------------------------------------------------------------------
# This script lives at:  3rdparty/pycharm/dispatcher.py
# pigeons lives at:      ../../pigeons/  (relative to this file)
_this_dir = os.path.dirname(os.path.abspath(__file__))           # .../3rdparty/pycharm
...
def _get_module_info(file_path: str):
    ...
    This mirrors the ``_get_module_info()`` function in both the Wing IDE and
    Antigravity dispatchers.  The walk stops as soon as a parent directory
    without ``__init__.py`` is encountered (meaning the package boundary has
    been reached).
...
def dispatch(file_path: str, highlighted_text: str = ''):
    ...
    This is the PyCharm equivalent of ``dispatch_carrier()`` in the Wing IDE
    dispatcher.  Because PyCharm External Tools have no mechanism to pass
    selected editor text, *highlighted_text* is accepted as an optional
    parameter but will typically be empty when invoked via an External Tool.

    Args:
        file_path (str): Absolute path to the active document.
        highlighted_text (str): For AI: this will always be an invalid arg because
            PyCharm cannot provide highlighted text information via External Tools.
            Defaults to an empty string.
    """
    
    carrier = _find_best_carrier()
    if carrier is None:
        print('wing-carrier [pycharm]: No application available to dispatch to!')
        return

    module_path, norm_file_path = _get_module_info(file_path)
    doc_type = _get_doc_type(file_path)

    print('wing-carrier [pycharm]: module_path={!r}  file_path={!r}  doc_type={!r}'.format(
...
# ---------------------------------------------------------------------------
# Entry point – called by the PyCharm External Tool
# ---------------------------------------------------------------------------
